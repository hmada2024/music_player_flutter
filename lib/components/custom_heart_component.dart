...
class FavouriteWidget extends StatefulWidget {
  final String id; // جعل الحقل نهائيًا
  const FavouriteWidget({super.key, required this.id});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Icon(
          Icons.favorite,
          size: favouriteIds!.contains(widget.id) ? 40 : 30,
          color: favouriteIds!.contains(widget.id) ? Colors.red : Colors.grey[400],
        ),
      ),
    );
  }
